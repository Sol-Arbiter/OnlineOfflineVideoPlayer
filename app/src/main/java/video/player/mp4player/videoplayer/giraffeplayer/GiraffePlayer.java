...
public class GiraffePlayer {
    ...
    private final View.OnClickListener onClickListener = new View.OnClickListener() {
        ...
        public void onClick(View v) {
            if (v.getId() == R.id.app_video_resize) {
                toggleFullScreen();
    ...
    public void setTitle(CharSequence title) {
        ...
        $.id(R.id.eq_app_video_title).text(title);
    }
    ...
    public void show(int timeout) {
        if (!isShowing) {
            ...
            if (!fullScreenOnly) {
                $.id(R.id.app_video_resize).visible();
            }
            isShowing = true;
    ...
    private void showBottomControl(boolean show) {
        ...
        $.id(R.id.btn_toggle_ratio).visibility(show ? View.VISIBLE : View.GONE);
        $.id(R.id.img_lock).visibility(show ? View.VISIBLE : View.GONE);
        $.id(R.id.img_volume).visibility(show ? View.VISIBLE : View.GONE);
        $.id(R.id.img_mini).visibility(show ? View.VISIBLE : View.GONE);
    }
    ...
    public GiraffePlayer(final Activity activity) {
        ...
        imageViews = (ImageView) activity.findViewById(R.id.imageView);
//        subtitles = (TextView) activity.findViewById(R.id.txtSubtitles);
        retriever = new MediaMetadataRetriever();
        imageViews.setScaleType(ImageView.ScaleType.CENTER_CROP);
        seekBar.setMax(1000);
        seekBar.setOnSeekBarChangeListener(mSeekListener);
        $.id(R.id.app_video_play).clicked(onClickListener);
        $.id(R.id.app_video_resize).clicked(onClickListener);
    ...
    private void hideAll() {
        PlayerActivity.play_activity.getWindow().setFlags(WindowManager.LayoutParams.FLAG_FULLSCREEN, WindowManager.LayoutParams.FLAG_FULLSCREEN);
        $.id(R.id.app_video_replay).gone();
        $.id(R.id.app_video_top_box).gone();
        $.id(R.id.app_video_loading).gone();
        $.id(R.id.app_video_resize).invisible();
        $.id(R.id.app_video_status).gone();
        $.id(R.id.img_lock).gone();
        $.id(R.id.img_volume).gone();
    ...
    public void function_onResume(int position) {
...
/*
            final TextView subView = (TextView) activity.findViewById(R.id.subs_box);

            videoView.ijkMediaPlayer.setOnTimedTextListener(new IMediaPlayer.OnTimedTextListener() {
                @Override
                public void onTimedText(IMediaPlayer mp, final IjkTimedText text) {
                    if (text != null) {
                        handler.post(new Runnable() {
                            @Override
                            public void run() {
//                                int seconds = (int) (mp.getCurrentPosition() / 1000);
                                subView.setText(text.getText());
                            }
                        });
                    }
                }
...
*/
/*
            subView.setPlayer(ijkMediaPlayer);
            subView.setSubSource(R.raw.sub, IjkMediaPlayer.MEDIA_MIMETYPE_TEXT_SUBRIP);
*/
        }
    }
    ...
    private void onVolumeSlide(float percent) {
        ...
        // 变更进度条
        int i = (int) (index * 1.0 / mMaxVolume * 200);
        String s = i + "%";
        if (i == 0) {
            s = "off";
        }
    ...
    private void updateFullScreenButton() {
        if (getScreenOrientation() == ActivityInfo.SCREEN_ORIENTATION_LANDSCAPE) {
            $.id(R.id.app_video_resize).image(R.drawable.ic_rotate);
        } else {
            $.id(R.id.app_video_resize).image(R.drawable.ic_rotate);
        }
    }
    ...
    public void update(int i) {
        int temp = i / 10;
        Long newPosition = Long.valueOf(totalDuration * temp);
        Bitmap bitmap = retriever.getFrameAtTime(newPosition * 10);
        imageViews.setImageBitmap(bitmap);
        int x = seekBar.getThumb().getBounds().centerX();
        int newPos = x + (i * imageViews.getWidth() / 1000);
        imageViews.setX(newPos);
    }

    /*
    private int findTrackIndexFor(int mediaTrackType, ITrackInfo[] trackInfo) {
        int index = -1;
